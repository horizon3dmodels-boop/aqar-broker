export async function getLocationByIP(): Promise<{ lat: number; lng: number; city: string; country: string }> {
  if (typeof navigator !== 'undefined' && navigator.geolocation) {
    try {
      ...
      return { lat: pos.coords.latitude, lng: pos.coords.longitude, city: "", country: "" };
    } catch {}
  }
  ...
  try {
    ...
    const location = { lat: data.latitude || 24.7136, lng: data.longitude || 46.6753, city: data.city || "", country: data.country_name || "" };
