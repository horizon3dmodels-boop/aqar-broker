...
const specialties = ["الكل", "بناء وتشييد", "تشطيب وديكور", "بنية تحتية", "طاقة شمسية", "صيانة وترميم", "تصميم داخلي"];
...
export default function ContractorsPage() {
  ...
  const filtered = contractors.filter((c) => {
    ...
    return c.full_name?.includes(searchText) || c.bio?.includes(searchText) || c.city?.includes(searchText);
  });
    ...
    <div dir="rtl" style={{ fontFamily: "'Cairo', sans-serif", minHeight: "100vh", background: "#F8F9FB" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .filter-btn:hover { background: #f0fdf4 !important; color: #16a34a !important; border-color: #16a34a !important; }
        input:focus, select:focus { border-color: #16a34a !important; box-shadow: 0 0 0 3px rgba(22,163,74,0.1) !important; outline: none; }
      `}</style>

      {/* Hero */}
      <section style={{ background: "linear-gradient(135deg, #052e16, #16a34a)", padding: "40px 24px", textAlign: "center" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          <h1 style={{ fontSize: 32, fontWeight: 900, color: "#fff", marginBottom: 10 }}>🔧 المقاولون المعتمدون</h1>
          <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 15, marginBottom: 24 }}>تواصل مع أفضل مقاولي البناء والتشطيب والصيانة</p>
          <div style={{ display: "flex", gap: 10, background: "#fff", borderRadius: 16, padding: 10, maxWidth: 600, margin: "0 auto" }}>
            <input
              type="text"
              placeholder="🔍 ابحث عن مقاول أو تخصص..."
              value={searchText}
            ...
            />
            <button style={{ background: "#16a34a", color: "#fff", border: "none", borderRadius: 10, padding: "10px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "'Cairo', sans-serif" }}>
              ابحث
            </button>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "24px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "20px 24px", marginBottom: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid #F0F0F0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {specialties.map((s) => (
                <button key={s} onClick={() => setSelectedSpecialty(s)} className="filter-btn" style={{
                  padding: "7px 16px", borderRadius: 20, border: "1.5px solid",
                  borderColor: selectedSpecialty === s ? "#16a34a" : "#E5E7EB",
                  background: selectedSpecialty === s ? "#16a34a" : "#fff",
                  color: selectedSpecialty === s ? "#fff" : "#6B7280",
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "#374151", cursor: "pointer" }}>
              <input type="checkbox" checked={verifiedOnly} onChange={(e) => setVerifiedOnly(e.target.checked)} style={{ accentColor: "#16a34a", width: 16, height: 16 }} />
              موثّق فقط ✓
            </label>
          </div>
        </div>
        ...
        <p style={{ marginBottom: 16, fontSize: 13, color: "#6B7280", fontWeight: 600 }}>
          {loading ? "جاري التحميل..." : `${filtered.length} مقاول متاح`}
        </p>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px", color: "#9CA3AF" }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 700 }}>لا يوجد مقاولون مسجلون حالياً</p>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 18 }}>
            {filtered.map((c) => (
              <div key={c.id} className="card-hover"
                ...
                <div style={{ position: "relative", height: 160, background: "linear-gradient(135deg, #052e16, #16a34a)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                  {c.avatar_url ? (
                  ...
                  <button onClick={(e) => { e.stopPropagation(); toggleLike(c.id); }}
                ...
                <div style={{ padding: "18px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                    <div style={{ width: 48, height: 48, borderRadius: "50%", background: "linear-gradient(135deg, #16a34a, #22c55e)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "2px solid #fff", marginTop: -28, boxShadow: "0 2px 8px rgba(0,0,0,0.15)", overflow: "hidden" }}>
                      {c.avatar_url ? (
                        <img src={c.avatar_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      ) : (
                        <span style={{ color: "#fff", fontWeight: 800, fontSize: 18 }}>{c.full_name?.[0] || "م"}</span>
                      )}
                    </div>
                    <div>
                      <h3 style={{ fontSize: 15, fontWeight: 800, color: "#0f172a" }}>{c.full_name}</h3>
                      <p style={{ fontSize: 12, color: "#6B7280" }}>📍 {c.city || "غير محدد"}</p>
                    </div>
                  </div>
                  ...
                  {c.commercial_register && (
                    <div style={{ background: "#F0FDF4", borderRadius: 8, padding: "6px 12px", fontSize: 11, color: "#16a34a", fontWeight: 700, marginBottom: 12, display: "inline-block" }}>
                      📋 سجل تجاري: {c.commercial_register}
                    </div>
                  )}

                  <div style={{ borderTop: "1px solid #F3F4F6", paddingTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <button
                      onClick={(e) => { e.stopPropagation(); window.open(`https://wa.me/${c.phone}`, "_blank"); }}
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); router.push(`/contractors/${c.id}`); }}
                      style={{ padding: "8px 14px", background: "#16a34a", border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer", fontFamily: "'Cairo', sans-serif" }}>
                      عرض الملف
                    </button>
                  </div>
                </div>
