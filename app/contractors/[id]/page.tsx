...
export default function ContractorDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!data) { router.push("/contractors"); return; }
      setContractor(data);
    ...
    <div dir="rtl" style={{ fontFamily: "'Cairo', sans-serif", minHeight: "100vh", background: "#F8F9FB" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #F0F0F0", padding: "12px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", fontSize: 13, color: "#9CA3AF", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span>←</span>
          <a href="/contractors" style={{ color: "#16a34a", textDecoration: "none" }}>المقاولون</a>
          <span>←</span>
          <span style={{ color: "#374151", fontWeight: 600 }}>{contractor.full_name}</span>
        </div>
      </div>

      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #052e16, #16a34a)", padding: "40px 24px 60px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", gap: 24 }}>
          <div style={{ width: 80, height: 80, borderRadius: "50%", background: "rgba(255,255,255,0.2)", border: "3px solid rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", flexShrink: 0 }}>
            {contractor.avatar_url ? (
              <img src={contractor.avatar_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            ) : (
              <span style={{ color: "#fff", fontSize: 32, fontWeight: 900 }}>{contractor.full_name?.[0] || "م"}</span>
            )}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              <h1 style={{ fontSize: 24, fontWeight: 900, color: "#fff" }}>{contractor.full_name}</h1>
              {contractor.verified && (
            ...
            <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 14, marginBottom: 4 }}>
              🔧 مقاول · 📍 {contractor.city || "غير محدد"} · 👥 {followersCount} متابع
            </p>
            {totalReviews > 0 && (
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {currentUser?.id !== params.id && (
              <button onClick={toggleFollow}
                style={{ background: followed ? "rgba(255,255,255,0.15)" : "#fff", color: followed ? "#fff" : "#052e16", border: followed ? "1.5px solid rgba(255,255,255,0.4)" : "none", borderRadius: 12, padding: "12px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Cairo', sans-serif" }}>
                {followed ? "✓ متابَع" : "+ تابع"}
              </button>
            )}
            <button onClick={() => window.open(`https://wa.me/${contractor.phone}`, "_blank")}
            ...
            <button onClick={() => window.location.href = `tel:${contractor.phone}`}
      ...
      <div style={{ maxWidth: 1280, margin: "-20px auto 0", padding: "0 24px 40px", display: "flex", gap: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "20px 24px", marginBottom: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", border: "1px solid #F0F0F0" }}>
            ...
            <div style={{ display: "flex", gap: 4, background: "#F3F4F6", padding: 4, borderRadius: 12, width: "fit-content", marginBottom: 20 }}>
              {[
                { id: "about", label: "📋 نبذة عن المقاول" },
              ...
              <div>
                {contractor.bio ? (
                  <div style={{ marginBottom: 24 }}>
                    <h3 style={{ fontSize: 15, fontWeight: 800, color: "#0f172a", marginBottom: 10 }}>عن المقاول</h3>
                    <p style={{ fontSize: 14, color: "#4B5563", lineHeight: 2 }}>{contractor.bio}</p>
                  </div>
                ) : (
                  <p style={{ fontSize: 14, color: "#9CA3AF", textAlign: "center", padding: "20px" }}>لم يضف المقاول نبذة بعد.</p>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }}>
                  {contractor.commercial_register && (
                    <div style={{ background: "#F0FDF4", borderRadius: 14, padding: "16px", border: "1px solid #BBF7D0" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>📋 {contractor.commercial_register}</div>
                    </div>
                  )}
                  {contractor.phone && (
                    <div style={{ background: "#F0F9FF", borderRadius: 14, padding: "16px", border: "1px solid #BAE6FD" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>📞 {contractor.phone}</div>
                    </div>
                  )}
                  {contractor.city && (
                    <div style={{ background: "#FFFBEB", borderRadius: 14, padding: "16px", border: "1px solid #FDE68A" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>📍 {contractor.city}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                {reels.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "40px", color: "#9CA3AF" }}>
                    ...
                    <p style={{ fontSize: 14, fontWeight: 700 }}>لم ينشر المقاول أي Reels بعد</p>
                  </div>
                ) : (
                      ...
                      <div key={r.id} style={{ background: "#F8F9FB", borderRadius: 14, padding: "16px", border: "1px solid #F0F0F0" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg, #16a34a, #22c55e)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
        ...
        <div style={{ width: 300, flexShrink: 0 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "20px", marginBottom: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", border: "1px solid #F0F0F0" }}>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: "#0f172a", marginBottom: 14 }}>تواصل مع المقاول</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <button onClick={() => window.location.href = `tel:${contractor.phone}`}
                style={{ width: "100%", background: "#16a34a", color: "#fff", border: "none", borderRadius: 12, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "'Cairo', sans-serif" }}>
                📞 اتصل الآن
              </button>
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.07)", border: "1px solid #F0F0F0" }}>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: "#0f172a", marginBottom: 12 }}>⭐ قيّم المقاول</h3>
            {totalReviews > 0 && (
            ...
            {reviewSent ? (
              ...
              <div>
                <p style={{ fontSize: 12, color: "#6B7280", marginBottom: 10, textAlign: "center" }}>كيف تقيّم هذا المقاول؟</p>
